...
"""
Moss Seed — Spatially Aware Gradio Simulator (v0.4)
Author: Nova (for Kumail / AI Cadmey)

WHAT'S NEW (Spatial Awareness):
- Local microclimate fields:
  • Moisture field M(x,y) that DIFFUSES, EVAPORATES, and DRIFTS with WIND.
  • Light field L(x,y) with a directional gradient + SHADING from biomass.
- Gradient‑following colonization:
  • Empty cells get colonized preferentially "up" the local suitability gradient,
    using 8‑direction advantage terms (source->target suitability gain).
- Optional SPORE dispersal downwind for long‑range jumps.

UI Controls:
- Environment sliders (humidity, light, temperature, pH, nutrients)
- Wet–dry cycles (period, amplitude)
- Spatial awareness toggle + parameters:
  - Shade coeff, Moisture diffusion, Evaporation, Wind (dir/speed)
  - Light gradient strength & Sun direction
  - Gradient bias (how strongly it climbs suitability)
  - Spore burst rate & travel (downwind distance)

Run:
  pip install gradio numpy matplotlib
  python moss_seed_gradio_spatial.py
"""
...
DIR_VECS /= np.linalg.norm(DIR_VECS, axis=1, keepdims=True)  # unit vectors
...
def suitability_raw(env):
    ...
    return 2.0 - penalty  # higher is better
...
def make_light_field(base_light, grad_strength, sun_deg):
    """Directional light gradient centered at grid center."""
    H, W = GRID_SIZE, GRID_SIZE
    y = np.linspace(-1, 1, H)
    x = np.linspace(-1, 1, W)
    X, Y = np.meshgrid(x, y)
    # sun vector (deg: 0=east/right, 90=north/up)
    th = np.deg2rad(sun_deg)
    vx, vy = np.cos(th), np.sin(th)
    g = grad_strength * (vx*X + vy*Y)  # projection
...
def diffuse(arr, k):
    """Simple 2D diffusion using 4-neighbor Laplacian approximation."""
    if k <= 0:
        return arr
...
def advect(arr, wind_deg, alpha):
    """Crude advection: drift field one cell toward wind direction by mix factor alpha."""
    if alpha <= 0:
        return arr
    th = np.deg2rad(wind_deg % 360)
    # map angle to nearest cardinal step
    dx = int(round(np.sin(th)))   # y axis increases downward
    dy = int(round(np.cos(th)))   # x axis
...
class MossSim:
    def __init__(self):
        ...
        self.reset_fields(PRESETS[DEFAULT_PRESET])
        self.step_id = 0
        self.running = False
    ...
    def reset_fields(self, env):
        ...
        # moisture & light fields
        self.M = np.full_like(self.grid, fill_value=env['humidity'], dtype=float)
        self.L0 = make_light_field(env['light'], grad_strength=0.0, sun_deg=0.0)  # base; will rebuild each step
        self.step_id = 0
    ...
    def spore_burst(self):
        if self.spore_rate <= 0:
            return
        if np.random.rand() > self.spore_rate:
            return
...
# Build UI
with gr.Blocks(title="Moss Seed — Spatially Aware Simulator") as demo:
    gr.Markdown("# 🌱 Moss Seed — Spatially Aware Growth Simulator")
    gr.Markdown("Seed starts at the center. Choose an environment, **toggle spatial awareness**, then Start. "
                "When spatial is ON, the moss senses **local moisture/light**, climbs **suitability gradients**, "
                "casts **shade**, and occasionally disperses **spores downwind**.")
    ...
    demo.load(fn=tick, outputs=[canvas, status])
    gr.Timer(0.15).tick(fn=tick, outputs=[canvas, status])
