...
"""
Moss Seed — Spatially Aware Simulator (v0.7, Smooth Playback)
Author: Nova (for Kumail / AI Cadmey)

Changes in v0.7
- ✅ Smooth playback: exactly **1 step per frame** (no batched jumps).
- ✅ Speed control now = **Playback FPS** (2–33 fps). Higher = faster visuals.
- ✅ Click-to-seed preserved: click anywhere to plant a new seed patch.
- ⏱️ Internal timer fixed to ~33 fps; slider throttles stepping without batching.

Run:
  pip install gradio numpy matplotlib
  python moss_seed_gradio_spatial_v0_7.py
"""
...
def suitability_raw(env):
    # env values may be scalars or arrays
...
class MossSim:
    def __init__(self):
        ...
        # playback control (no batching): base timer ~33 fps -> skip_n sets effective fps
        self.BASE_FPS = 33.0
        self.skip_n = 2  # default → ~16 fps
        self._tick_count = 0
    ...
    def set_fps(self, fps):
        ...
        # skip every k ticks to approximate target fps without batching
    ...
    def plant_seed_rc(self, r, c, radius=1):
    ...
    def step(self):
        ...
        S = logistic(suitability_raw(local_env))
        neigh = neighborhood_mean(self.grid)

        # Colonization (spatial gradient aware)
...
def set_playback_fps(fps):
    SIM.set_fps(fps)
    cov, mb = SIM.coverage(), SIM.mean_biomass()
    return f"Playback FPS ≈ {float(fps):.0f} (1 step/frame, no batching) | Step {SIM.step_id} | Coverage {cov:.2f} | Mean biomass {mb:.2f}"

def plant_on_click(evt: gr.SelectData):
    try:
        x, y = evt.index  # (col, row) in image coords
    except Exception:
...
def tick(_dummy=None):
    # Fixed-rate timer (~33 Hz). Advance **at most one** step each tick depending on skip_n.
    SIM._tick_count += 1
...
# Build UI
with gr.Blocks(title="Moss Seed — Smooth Playback (Click + FPS)") as demo:
    gr.Markdown("# 🌱 Moss Seed — Spatial + Click-to-Seed + **Smooth Speed**")
    gr.Markdown("**One simulation step per frame** — increasing speed only raises the frame rate (no step jumping). "
                "Click on the image to plant new seeds anywhere.")
    ...
    with gr.Accordion("Spatial Awareness & Fields", open=False):
        with gr.Row():
    ...
    with gr.Row():
        fps = gr.Slider(2, 33, value=16, step=1, label="Playback Speed — Frames per Second (1 step/frame)")
    ...
    # Fixed-rate timer ~33 Hz; tick enforces skip_n to match target FPS
    demo.load(fn=tick, outputs=[canvas, status])
    gr.Timer(0.03).tick(fn=tick, outputs=[canvas, status])
